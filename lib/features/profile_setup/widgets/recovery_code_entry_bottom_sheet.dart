...
class RecoveryCodeEntryBottomSheet extends StatefulWidget {
  ...
  @override
  State<RecoveryCodeEntryBottomSheet> createState() => _RecoveryCodeEntryBottomSheetState();
}

class _RecoveryCodeEntryBottomSheetState extends State<RecoveryCodeEntryBottomSheet> with TickerProviderStateMixin {
  ...
  bool get _isNameValid {
    ...
    return name.split(' ').where((word) => word.isNotEmpty).length >= 3 && !_hasEnglishInName;
  }
  ...
  Future<void> _submitRecoveryRequest() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        // Show error in description with typewriter effect
        _startTypewriterEffect('کۆدی گەڕانەوە هەڵەیە', isError: true);
      }
    } catch (e) {
      setState(() {
        _isLoading = false;
                    ...
                    child: Column(
                      ...
                      children: [
                                ...
                                Container(
                                  ...
                                  child: TextField(
                                    ...
                                    onChanged: (value) {
                                      ...
                                      setState(() {
                                        // If user has 8 digits and deletes one (becomes 7), clear error
                                        if (_hasError && digitsOnly.length < 8 && _isShowingErrorMessage) {
                                          _hasError = false;
                                ...
                                Container(
                                  ...
                                  child: TextField(
                                    ...
                                    onChanged: (value) {
                                      ...
                                      final previousHasEnglish = _hasEnglishInName;
                        ...
                        Material(
                          ...
                          child: InkWell(
                            onTap: (_isLoading || !_isFormValid) ? null : _submitRecoveryRequest,
                            borderRadius: BorderRadius.circular(10 * scale),
                            child: Ink(
                              decoration: BoxDecoration(
                            ...
                            child: Column(
                              children: [
                                _buildInfoSection(
                                  ...
                                  content: 'کۆدی گەڕانەوە ژمارەیەکی تایبەتە کە لە کاتی دروستکردنی هەژمارەکەت پێت دراوە بۆ گەڕانەوەی هەژمارەکەت لە ئامێرێکی تر یان لە کاتی سڕینەوە',
                                  scale: scale,
                                ),
                                _buildInfoSection(
                                  sectionId: 'why_name',
                                  title: 'بۆچی پێویستە ناوم بنووسم ؟',
                                  content: 'ناوەکەت بۆ دڵنیابوون لە ئەوەیە کە تۆ خاوەنی ڕاستەقینەی ئەم هەژمارەی، داواکارییەکەت پێداچوونەوەی دەکرێت',
                                  scale: scale,
                                ),
                                _buildInfoSection(
                                  sectionId: 'how_long',
                                  title: 'چەند دەخایەنێت بۆ پەسەندکردن ؟',
                                  content: 'داواکارییەکەت لە ماوەی ٢٤ کاتژمێردا پێداچوونەوەی دەکرێت، ئاگادارت دەکەینەوە کاتێک پەسەندکرا یان ڕەتکرایەوە',
                                  scale: scale,
                                ),
                                _buildInfoSection(
                                  sectionId: 'what_after',
                                  title: 'دوای ناردنی داواکاری چی دەبێت ؟',
                                  content: 'داواکارییەکەت چاوەڕوانی پەسەندکردن دەبێت، دەتوانیت بارودۆخەکە لە ئەپەکە ببینیت',
                                  scale: scale,
                                ),
                                _buildInfoSection(
                                  sectionId: 'lost_code',
                                  title: 'ئەگەر کۆدەکەم ونبوو چی بکەم ؟',
                                  content: 'بەداخەوە ئەگەر کۆدەکەت ونبوو، ناتوانیت هەژمارەکەت بگەڕێنیتەوە، پێویستە هەژمارێکی نوێ دروست بکەیت',
                                  scale: scale,
                                ),
                              ],
                            ),
  ...
  Widget _buildColoredText(String text, double scale) {
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      spans.add(
        TextSpan(
          text: i < parts.length - 1 ? parts[i] + (i % 2 == 0 ? '"' : '') : parts[i],
          style: TextStyle(
            fontFamily: 'Peshang',
            fontSize: 10 * scale,
            color: color,
